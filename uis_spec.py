# -*- coding: utf-8 -*-
"""
Class for storing dimension-value pairs relating to a UIS indicator
@author: scameron

"""

class UISSpec(object):
    ...
    def __init__(self, **kwargs):
        ...
        for k, v in kwargs.items():
            if k.upper() in self.dimension_ids:
    ...
    @classmethod
    def dims(cls):
        return [d for d in cls.dimension_ids if d not in cls.ignore]
    
    @classmethod
    def from_key(cls, k):
        """ Creates a specification from an SDMX key 
        e.g. """
        return cls(**dict(zip(cls.dimension_ids, k.split("."))))
    ...
    def to_key(self, ref_area=False, time_period=False):
        ...
        for dimension_id in self.dimension_ids:
            if not((dimension_id == "REF_AREA" and ref_area is False) or
                ...
                v = d.get(dimension_id.upper(), d.get(dimension_id.lower(), None))
                if v is None:
                    r.append("")
                elif type(v) == list:
                    r.append('+'.join(v))
